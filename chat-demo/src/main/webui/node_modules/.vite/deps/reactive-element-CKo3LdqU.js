...
*/
var supportsAdoptingStyleSheets = global$1.ShadowRoot && (global$1.ShadyCSS === void 0 || global$1.ShadyCSS.nativeShadow) && "adoptedStyleSheets" in Document.prototype && "replace" in CSSStyleSheet.prototype;
...
var polyfillSupport = global.reactiveElementPolyfillSupportDevMode;
...
queueMicrotask(() => {
	...
	if (global.ShadyDOM?.inUse && polyfillSupport === void 0) issueWarning("polyfill-support-missing", "Shadow DOM is being polyfilled via `ShadyDOM` but the `polyfill-support` module has not been loaded.");
});
/**
...
var debugLogEvent = (event) => {
	if (!global.emitLitDebugLogEvents) return;
	global.dispatchEvent(new CustomEvent("lit-debug", { detail: event }));
};
...
global.litPropertyMetadata ??= /* @__PURE__ */ new WeakMap();
/**
...
var ReactiveElement = class extends HTMLElement {
	...
	*/
	static finalize() {
		...
		if (this.hasOwnProperty("createProperty")) issueWarning("no-override-create-property", "Overriding ReactiveElement.createProperty() is deprecated. The override will not be called with standard decorators");
		if (this.hasOwnProperty("getPropertyDescriptor")) issueWarning("no-override-get-property-descriptor", "Overriding ReactiveElement.getPropertyDescriptor() is deprecated. The override will not be called with standard decorators");
	}
	/**
	...
	*/
	...
	__propertyToAttribute(name, value) {
		...
		if (attr !== void 0 && options.reflect === true) {
			...
			if (this.constructor.enabledWarnings.includes("migration") && attrValue === void 0) issueWarning("undefined-attribute-value", `The attribute value for the ${name} property is undefined on element ${this.localName}. The attribute will be removed, but in the previous version of \`ReactiveElement\`, the attribute would not have changed.`);
	...
	*/
	requestUpdate(name, oldValue, options, useNewValue = false, newValue) {
		if (name !== void 0) {
			if (name instanceof Event) issueWarning(``, `The requestUpdate() method was called with an Event as the property name. This is probably a mistake caused by binding this.requestUpdate as an event listener. Instead bind a function that will call it with no arguments: () => this.requestUpdate()`);
			const ctor = this.constructor;
	...
	*/
	scheduleUpdate() {
		...
		if (this.constructor.enabledWarnings.includes("async-perform-update") && typeof result?.then === "function") issueWarning("async-perform-update", `Element ${this.localName} returned a Promise from performUpdate(). This behavior is deprecated and will be removed in a future version of ReactiveElement.`);
		return result;
	}
	/**
	...
	*/
	...
	_$didUpdate(changedProperties) {
		...
		if (this.isUpdatePending && this.constructor.enabledWarnings.includes("change-in-update")) issueWarning("change-in-update", `Element ${this.localName} scheduled an update (generally because a property was set) after an update completed, causing a new update to be scheduled. This is inefficient and should be avoided unless the next update can only be scheduled as a side effect of the previous update.`);
	}
	__markUpdated() {
		this._$changedProperties = /* @__PURE__ */ new Map();
		this.isUpdatePending = false;
...
polyfillSupport?.({ ReactiveElement });
{
	...
	const ensureOwnWarnings = function(ctor) {
		if (!ctor.hasOwnProperty(JSCompiler_renameProperty("enabledWarnings", ctor))) ctor.enabledWarnings = ctor.enabledWarnings.slice();
	};
...
if (global.reactiveElementVersions.length > 1) queueMicrotask(() => {
	issueWarning("multiple-versions", "Multiple versions of Lit loaded. Loading multiple versions is not recommended.");
});
//#endregion
export { adoptStyles as a, supportsAdoptingStyleSheets as c, CSSResult as i, unsafeCSS as l, defaultConverter as n, css as o, notEqual as r, getCompatibleStyle as s, ReactiveElement as t };
